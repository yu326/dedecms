    ...
    readCookie: function (l) {
        ...
        if (document.cookie.length > 0) {
            ...
            if (offset != -1) {
                ...
                i = document.cookie.substring(offset, end)
            }
        }
        ;
        return i
...
var epidiascope = {
    ...
    init: function () {
        ...
        //
        sina.$(this.picArrLeftId).onclick = function () {
            epidiascope.previous();
            epidiascope.stop();
        };
        sina.$(this.picArrRightId).onclick = function () {
            epidiascope.next();
            epidiascope.stop();
        };
    ...
    select: function (num, type) {
        ...
        ;
        var i;
        if (num >= this.filmstrips.length || num < 0) {
            return
        }
        ;
        ...
        if (sina.$("slide_" + this.selectedIndex)) {
    ...
    PVCount: function (type) {
        if (type == "auto") {
            if (this.PVUrl_a == null) {
                return;
            }
            ;
        } else {
            if (this.PVUrl_m == null) {
                return;
            }
            ;
        }
    ...
    play: function () {
        ...
        sina.$(this.playButtonId).onclick = function () {
            epidiascope.stop()
        };
    ...
    listSelect: function (num) {
        if (num < 0 || num >= this.listData.length) {
            return
        }
        ;
...
epidiascope.speedBar = { //速度条
    ...
    init: function () {
        ...
        sina.$(this.slideId).onselectstart = function () {
            return false
        };
        sina.$(this.slideButtonId).onmousedown = function (e) {
            tempThis.mouseDown(e);
            return false
        };
        sina.$(this.slideId).onmousedown = function (e) {
            tempThis.slideClick(e);
            return false
        };
...
epidiascope.picList = { //列表滚动
    ...
    lock: false,
    init: function () {
        sina.$(this.rightArrId).onmousedown = function () {
            epidiascope.picList.leftMouseDown()
        };
        sina.$(this.rightArrId).onmouseout = function () {
            epidiascope.picList.leftEnd("out");
            this.className = '';
        };
        sina.$(this.rightArrId).onmouseup = function () {
            epidiascope.picList.leftEnd("up")
        };
        sina.$(this.leftArrId).onmousedown = function () {
            epidiascope.picList.rightMouseDown()
        };
        sina.$(this.leftArrId).onmouseout = function () {
            epidiascope.picList.rightEnd("out");
            this.className = '';
        };
        sina.$(this.leftArrId).onmouseup = function () {
    ...
    moveRight: function () {
        sina.$(this.picListId).scrollLeft -= 10;
        if (sina.$(this.picListId).scrollLeft == 0) {
            this.rightEnd()
        }
        ;
    },
    ...
    move: function (num) {
        ...
        if (temp <= 0) {
            sina.$(this.picListId).scrollLeft = 0;
            this.lock = false;
            return;
        }
        if (temp >= this.totalWidth - this.offsetWidth) {
            sina.$(this.picListId).scrollLeft = this.totalWidth - this.offsetWidth;
            this.lock = false;
            return;
        }
...
epidiascope.keyboard = {
    ...
    init: function () {
        var tempThis = this;
        sina.addEvent(document, 'keydown', function (e) {
            tempThis.keyDown(e)
        });
...
epidiascope.endSelect = {
    ...
    open: function () {
        ...
        epidiascope.stop();
        sina.$(epidiascope.playButtonId).onclick = function () {
            epidiascope.rePlay()
        };
...
epidiascope.Button.prototype.init = function () {
    ...
    sina.addEvent(this.element, 'mouseover', function () {
        tempThis.mouseover()
    });
    sina.addEvent(this.element, 'mouseout', function () {
        tempThis.mouseout()
    });
    sina.addEvent(this.element, 'mousedown', function () {
        tempThis.mousedown()
    });
    sina.addEvent(this.element, 'mouseup', function () {
        tempThis.mouseup()
    });
};
...
function DivSelect(O, l, I) {
    ...
    for (var c = 0; c < C.selectObj.options.length; c++) {
        ...
        if (C.selectObj.selectedIndex == c) {
            C.divObj_title.innerHTML = i.innerHTML
        }
        ;
    ...
    C.select = function (i) {
        ...
        for (var I = 0; I < l.selectObj.options.length; I++) {
            if (l.selectObj.options[I].innerHTML == i) {
                ...
                if (l.selectObj.onchange) {
                    l.selectObj.onchange()
                }
                ;
                l.divObj_title.innerHTML = i;
                break
            }
        }
    };
